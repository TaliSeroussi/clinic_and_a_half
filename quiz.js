...
var arr_questions_bank_1 = [
  {
    ...
    wrong_answer: [
      `מזגן, פחים, טלוויזיה, שילוט המורה על "איסור עישון" ואמצעי הגנה מפגעי מזג האוויר.`,
      `מזגן, ספסלים, פחים, טלוויזיה בה מוצגים נהלי הרפואה ביחידה ואמצעי הגנה מפגעי מזג האוויר.`,
      `ספסלים, פחים, טלוויזיה, שילוט המורה על "איסור עישון" ואמצעי הגנה מפגעי מזג האוויר.`,
    ],
  },
  {
  ...
  },
  {
    ...
    wrong_answer: [
      ...
      `דרכי השגת הצוות הרפואי מחוץ לשעות הפעילות ובעת חירום`,
    ],
  },
  {
  ...
  {
    question:
      "איזה נושא <span class=`bold gray`>לא</span> נכלל בלוח הנחיות רפואת נשים?",
    correct_answer: `נוהל קבלת אמצעי מניעה`,
    wrong_answer: [`נוהל בדיקת חיילת`, `רשימת רופאות נשים`, `נוהל פניה למהו"ת`],
  },
  {
    question:
      "איזה נושא <span class=`bold gray`>לא</span> נכלל בלוח הנחיות רפואה כלליות?",
    correct_answer: `נוהל בדיקת חייל מעודכן`,
    wrong_answer: [
      `נוהל פנייה לוועדה רפואית`,
      `נוהל פנייה של משרתי קבע לרפואת מומחים`,
      `דרכי תקשורת עם מרכז מידע וזימון תורים`,
    ],
  },
  {
    question:
      "איזה נוהל <span class=`bold gray`>לא</span> שייך ללוח רפואת מומחים?",
    correct_answer: `שעות פעילות המרפאה`,
    wrong_answer: [
      `נוהל הפניית חייל בחובה לרפואת מומחים`,
      `נוהל הפניית חייל בחובה/בקבע לקב"ן`,
      `רשימת מרפאות מומחים ודרכי השגתם`,
    ],
  },
  {
    ...
    wrong_answer: [
      `נוהל ביקורת תברואה יומית`,
      `רשימת מרפאות מומחים צבאיות ואזרחיות`,
      `הוראות קרפ"ר רלוונטיות`,
    ],
  },
  {
    ...
    wrong_answer: [
      `תלויים ברחבי המרפאה או מודפסים בעלונים שנמצאים בחדר קבלה`,
      `תלויים על גבי דלתות חדרי הרופאים`,
      `תלויים במרפאה בלבד`,
    ],
  }
];
...
var mat_questions_bank = [
  copy(arr_questions_bank_1),
  copy(arr_questions_bank_2),
  copy(arr_questions_bank_3),
  copy(arr_questions_bank_4),
];
...
first_question = () => {
  ...
  if (matrix[nRoom][nPage].questionType === "life") {
    $(".quiz-type").html(
      `בוחן עבור <img class="text-heart" src="assets/media/heart/heart1_happy.svg">`
    );
...
type_quiz = () => {
  $(`#${matrix[nRoom][nPage].divName} .question-counter`).text(
...
pop_insert_question = () => {
  ...
  // insert question
  $(`#${matrix[nRoom][nPage].divName} .questions`).html(
    question_bank[question_number].question
  );
  // fill answers
  for (let i = 1; i <= ANSWER_NUM; i++) {
    if (i === correct_answer) {
      $(`#${matrix[nRoom][nPage].divName} .answer.data-num-${i}`).text(
        question_bank[question_number].correct_answer
      );
      $(`#${matrix[nRoom][nPage].divName} .answer.data-num-${i}`).addClass(
        "correct"
      );
    } else {
      let wrong_answer = Math.floor(
        Math.random() * question_bank[question_number].wrong_answer.length
      );
      $(`#${matrix[nRoom][nPage].divName} .answer.data-num-${i}`).text(
        question_bank[question_number].wrong_answer[wrong_answer]
      );
      mat_questions_bank[nRoom - 1][question_number].wrong_answer.splice(
        wrong_answer,
        1
      );
    }
...
check_answer = (event) => {
  switch_class(
    $(`#${matrix[nRoom][nPage].divName} .correct`),
    "normal",
    "right"
  );
  ...
  if (correct_question_counter > question_num / 2) {
    ...
    if (matrix[nRoom][nPage].questionType === "life") {
      ...
      for (let i = 1; i <= nLife; i++) {
        $(`#heart-${i} .heart`).attr(
          "src",
          `assets/media/heart/heart${i}_happy.svg`
        );
      }
    ...
    // going back to home page to open new room
      ...
      $("#spinning-bg").attr(
        "src",
        "assets/media/room_finish/round_finish_bg.svg"
      );
      $(`#ending-room`).css("display", "block");
      $(`#room-item`).attr(
        "src",
        `assets/media/room_finish/finish_room_${nRoom}.svg`
      );
        ...
        if (arr_marks.length < 4) {
          ...
          matrix[nRoom].forEach((page) => {
            ...
            if (page.topic !== undefined && page.topic !== 1) {
              page.functions.push(
                `switch_class($("#back-button"), "hidden", "visible")`
              );
            }
          });
          homePage();
